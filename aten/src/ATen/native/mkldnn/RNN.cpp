...
struct RNNParams {
  ...
  desc src_layer_desc(int64_t _input_size, ScalarType data_type) const {
    return {{seq_length, mini_batch, _input_size}, get_mkldnn_dtype(data_type), format::tnc};
  }
  desc src_iter_desc(ScalarType data_type) const {
    return {{1, 1, mini_batch, hidden_size}, get_mkldnn_dtype(data_type), format::ldnc};
  }
  // logical size described as ldigo
  desc weights_layer_desc(int64_t _input_size, ScalarType data_type) const {
    return {{1, 1, _input_size, num_gates, hidden_size}, get_mkldnn_dtype(data_type), format::ldgoi};
  }
  desc weights_iter_desc(ScalarType data_type) const {
    return {{1, 1, hidden_size, num_gates, hidden_size}, get_mkldnn_dtype(data_type), format::ldgoi};
  }
  desc bias_desc(ScalarType data_type) const {
    return {{1, 1, num_bias_gates, hidden_size}, get_mkldnn_dtype(data_type), format::ldgo};
  }
  desc dst_layer_desc(ScalarType data_type) const {
    return {{seq_length, mini_batch, hidden_size}, get_mkldnn_dtype(data_type), format::tnc};
  }
  desc dst_iter_desc(ScalarType data_type) const {
    return {{1, 1, mini_batch, hidden_size}, get_mkldnn_dtype(data_type), format::ldnc};
  }
};
...
Tensor mkldnn_rnn_layer(Tensor& hy_, Tensor& cy_,
  ...
  ScalarType type = input.scalar_type();
  auto x = get_mkldnn_tensor(input, rnn.src_layer_desc(input_size, type));
  auto hx = get_mkldnn_tensor(hx_, rnn.src_iter_desc(type));
  auto cx = get_mkldnn_tensor(cx_, rnn.src_iter_desc(cx_.scalar_type()));
  auto w1 = get_mkldnn_tensor(weight_ih, rnn.weights_layer_desc(input_size, type));
  auto w2 = get_mkldnn_tensor(weight_hh, rnn.weights_iter_desc(type));
  auto b = get_mkldnn_tensor(bias, rnn.bias_desc(type));
  if (type == ScalarType::BFloat16) {
    auto f32_bias = at::empty(bias.sizes(), bias.options().dtype(ScalarType::Float));
    f32_bias.copy_(bias);
    b = get_mkldnn_tensor(f32_bias, rnn.bias_desc(f32_bias.scalar_type()));
  }
  auto y = get_mkldnn_tensor(output, rnn.dst_layer_desc(type));
  auto hy = get_mkldnn_tensor(hy_, rnn.dst_iter_desc(type));
  auto cy = get_mkldnn_tensor(cy_, rnn.dst_iter_desc(cy_.scalar_type()));
  //auto cy = get_mkldnn_tensor(cy_, rnn.dst_iter_desc(type == ScalarType::BFloat16 ? ScalarType::Float : type)));
